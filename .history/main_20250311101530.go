...
func sha256(msg []byte) [32]byte {
	...
	for i := 0; i < len(msg); i += 64 {
		...
		for j := 16; j < 64; j++ {
			s0 := bits.RotateRight32(w[j-15], 7) ^ bits.RotateRight32(w[j-15], 18) ^ (w[j-15] >> 3)
			s1 := bits.RotateRight32(w[j-2], 17) ^ bits.RotateRight32(w[j-2], 19) ^ (w[j-2] >> 10)
			w[j] = w[j-16] + s0 + w[j-7] + s1
		}

		a, b, c, d, e, f, g, hh := h[0], h[1], h[2], h[3], h[4], h[5], h[6], h[7]
		for j := 0; j < 64; j++ {
			t1 := hh + ((e>>6 | e<<26) ^ (e>>11 | e<<21) ^ (e>>25 | e<<7)) + ((e & f) ^ (^e & g)) + K[j] + w[j]
